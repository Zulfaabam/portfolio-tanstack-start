...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className='bg-dark w-full' ref={containerRef}>
      ...
      <div
        ref={ref}
        className='px-0! relative max-w-5xl space-y-4 pb-20 md:space-y-6'
      >
        {data.map((item, index) => (
        ...
        >
          <motion.div
            ...
            className='bg-linear-to-t from-primary via-secondary absolute inset-x-0 top-0 w-0.5 rounded-full from-0% via-10% to-transparent'
          />
        </div>
      </div>
    </div>
